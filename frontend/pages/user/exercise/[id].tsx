...
function AddQuizz() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const resQuestion = await exerciseApi.getQuestionById(
        currentQuizz,
        currentQuestion,
        config
      );
      const resOption = await exerciseApi.getOption(
        currentQuizz,
        currentQuestion,
        config
      );
      setOptions(resOption.data);
      const newOption = resOption.data.map(
        ({ id, ...newObject }: { id: any }) => newObject
      );
      setInitialValues({
  ...
  const handlePulish = async () => {
    await exerciseApi
      .confirmQuestion(currentQuizz, currentQuestion, config)
      .then();
    await exerciseApi
      .pulishQuestion(currentQuizz, currentQuestion, config)
    ...
    <HomeLayout>
      <div className="container" style={{ minHeight: "100vh" }}>
        ...
        <div className={styles.nav}>
            ...
            return (
              <div
                ...
                className={`${styles.navItem} ${
                  currentQuizz == quizz.id && styles.active
                }`}
              >
                {editQuizz === quizz.id ? (
                  <input
          ...
          <div>
            <PlusSquare className={styles.navAdd} onClick={handleAddQuiz} />
          </div>
        </div>
        {quizzs.length != 0 && (
          <div className={styles.nav}>
            {questions.map((question, index) => {
              return (
                <div
                  key={question.id}
                  className={`${styles.navItem} ${
                    currentQuestion == question.id && styles.active
                  }`}
                >
              ...
              <div className="mt-3">
                <Formik
                  ...
                  onSubmit={async (values) => {
                    const length =
                      values.options.length - initialValues.options.length;
                    ...
                    if (length > 0) {
                      ...
                      for (let i = 0; i < initialValues.options.length; i++) {
                        await exerciseApi.updateOption(
                          values.options[i],
                          currentQuizz,
                          currentQuestion,
                          options[i].id,
                      ...
                      for (
                        let i = initialValues.options.length;
                        i < values.options.length;
                        i++
                      ) {
                        await exerciseApi.postOption(
                          values.options[i],
                          currentQuizz,
                          currentQuestion,
                          config
                      ...
                      for (
                        let i = values.options.length;
                        i < initialValues.options.length;
                        i++
                      ) {
                        await exerciseApi.deleteOption(
                          currentQuizz,
                          currentQuestion,
                          options[i].id,
                          config
